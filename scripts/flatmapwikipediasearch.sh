folder=data/output_files/flatmapwikipediamuspe/
duration=60000
batch_size=1
allowed_lateness=30000
impl_types=(NATIVE NATIVE NATIVE NATIVE NATIVE NATIVE)
exp_ids=(LLF ALF HLF LHF AHF HHF) 
rates=(600000 600000 600000 250000 250000 180000)
max_diffs=(2000 2000 2000 2000 2000 2000)
parallelisms=(5 5 5 5 5 4)
		...
		for r in {0..0}; do
			...
			do
				...
				if [ ! -z $(grep "${exp_folder},${exp_id},${r},${rate},${sleepTime},1" "${folder}/log.txt") ]; then 
					...
					mvn compile exec:java -Dexec.mainClass=muSPE.FlatMapQuery -Dexec.args="--throughputOutputFile ${exp_folder}/throughput.csv --outputRateOutputFile ${exp_folder}/outputRate.csv --experimentID ${exp_id} --ack ${exp_folder}/completedsuccessfully.txt --latencyOutputFile ${exp_folder}/latency.csv --maxLatency 15000 --maxLatencyViolations 3 --duration ${duration} --parallelism ${parallelism} --sleepTime ${sleepTime} --injectionRateOutputFile ${exp_folder}/injectionRate.csv --inputFile data/input_files/insertions.tsv" > ${exp_folder}/muspelog.txt 2>&1 &
