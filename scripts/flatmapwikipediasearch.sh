folder=data/output_files/flatmapwikipediaflinkinterspe/
duration=120000
batch_size=1
allowed_lateness=30000
impl_types=(NATIVE NATIVE NATIVE NATIVE NATIVE NATIVE)
exp_ids=(LLF ALF HLF LHF AHF HHF) 
rates=(400000 400000 400000 200000 200000 200000)
max_diffs=(5000 5000 5000 5000 5000 5000)
		...
		for r in {0..0}; do
			...
			do
				...
				if [ ! -z $(grep "${exp_folder},${exp_id},${r},${rate},${sleepTime},1" "${folder}/log.txt") ]; then 
					...
					java --add-opens=java.base/java.lang=ALL-UNNAMED --add-opens=java.base/java.lang.invoke=ALL-UNNAMED --add-opens=java.base/java.lang.reflect=ALL-UNNAMED --add-opens=java.base/java.io=ALL-UNNAMED --add-opens=java.base/java.net=ALL-UNNAMED --add-opens=java.base/java.nio=ALL-UNNAMED --add-opens=java.base/java.util=ALL-UNNAMED --add-opens=java.base/java.util.concurrent=ALL-UNNAMED --add-opens=java.base/java.util.concurrent.atomic=ALL-UNNAMED --add-opens=java.base/sun.nio.ch=ALL-UNNAMED --add-opens=java.base/sun.nio.cs=ALL-UNNAMED --add-opens=java.base/sun.security.action=ALL-UNNAMED --add-opens=java.base/sun.util.calendar=ALL-UNNAMED --add-opens=java.security.jgss/sun.security.krb5=ALL-UNNAMED -classpath ./target/aggregates_for_the_win-1.0-SNAPSHOT.jar:$CLASSPATH usecase.wikipedia.QueryFlatMap --inputFile data/input_files/insertions.tsv --type native --injectionRateOutputFile ${exp_folder}/injectionRate.csv --throughputOutputFile ${exp_folder}/throughput.csv --latencyOutputFile ${exp_folder}/latency.csv --duration ${duration} --batchSize ${batch_size} --sleepTime ${sleepTime} --experimentID ${exp_id} --outputRateOutputFile ${exp_folder}/outputRate.csv --maxLatencyViolations 3 --maxLatency 15000 --implementationType ${type} --statsFolder ${exp_folder} > ${exp_folder}/flinklog.txt 2>&1 &
