folder=data/output_files/joinwikipediaspark/
duration=120000
batch_size=1
allowed_lateness=30000
impl_types=(NATIVE NATIVE NATIVE NATIVE NATIVE NATIVE)
exp_ids=(LLJ ALJ HLJ LHJ AHJ HHJ) 
rates=(8000 8000 8000 4800 4800 4800)
max_diffs=(200 200 200 200 200 200)
		...
		for r in {0..0}; do
			...
			do
				...
				if [ ! -z $(grep "${exp_folder},${exp_id},${r},${rate},${sleepTime},1" "${folder}/log.txt") ]; then 
					...
					nohup mvn compile exec:java -Dexec.mainClass=spark.StringServer -Dexec.args="--inputFile data/input_files/insertions.tsv --port 9999 --duration ${duration} --sleepTime ${sleepTime} --injectionRateOutputFile ${exp_folder}/injectionRate.csv --warmup 30000" > ${exp_folder}/sourcelog.txt 2>&1 &

					sleep 5

					mvn compile exec:java -Dexec.mainClass=spark.JoinTests -Dexec.args="--IP 129.16.20.20 --port 9999 --throughputOutputFile ${exp_folder}/throughput.csv --outputRateOutputFile ${exp_folder}/outputRate.csv --experimentID ${exp_id} --ack ${exp_folder}/completedsuccessfully.txt --latencyOutputFile ${exp_folder}/latency.csv --maxLatency 60000 --maxLatencyViolations 3 --duration ${duration} --sleepTime ${sleepTime} --injectionRateOutputFile ${exp_folder}/injectionRate.csv --outputFile ${exp_folder}/output.csv --warmup 30000 --inputFile data/input_files/insertions.tsv" > ${exp_folder}/sparklog.txt 2>&1 &
